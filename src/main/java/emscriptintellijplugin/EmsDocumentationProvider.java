 ...
 */
final class EmsDocumentationProvider extends AbstractDocumentationProvider {

  /**
   * For the Ems Language, we don't have online documentation. However, if your language provides
  ...
  /**
   * Extracts the key, value, file and documentation comment of a Ems key/value entry and returns
   * a formatted representation of the information.
   */
  @Override
  public @Nullable String generateDoc(PsiElement element, @Nullable PsiElement originalElement) {
    if (element instanceof EmsProperty) {
      final String key = ((EmsProperty) element).getKey();
      final String value = ((EmsProperty) element).getValue();
      final String file = SymbolPresentationUtil.getFilePathPresentation(element.getContainingFile());
      final String docComment = EmsUtil.findDocumentationComment((EmsProperty) element);
  ...
  /**
   * Provides the information in which file the Ems language key/value is defined.
   */
  @Override
  ...
  /**
   * Provides documentation when a Ems Language element is hovered with the mouse.
   */
  @Override
