...
final class EmsFoldingBuilder extends FoldingBuilderEx implements DumbAware {

  @Override
  public FoldingDescriptor @NotNull [] buildFoldRegions(@NotNull PsiElement root, @NotNull Document document, boolean quick) {
    // Initialize the group of folding regions that will expand/collapse together.
    FoldingGroup group = FoldingGroup.newGroup(EmsAnnotator.EMS_PREFIX_STR);
    ...
    root.accept(new JavaRecursiveElementWalkingVisitor() {
      ...
      public void visitLiteralExpression(@NotNull PsiLiteralExpression literalExpression) {
        ...
        if (value != null && value.startsWith(EmsAnnotator.EMS_PREFIX_STR + EmsAnnotator.EMS_SEPARATOR_STR)) {
          Project project = literalExpression.getProject();
          String key = value.substring(
              EmsAnnotator.EMS_PREFIX_STR.length() + EmsAnnotator.EMS_SEPARATOR_STR.length()
          );
          // find EmsProperty for the given key in the project
          EmsProperty emsProperty = ContainerUtil.getOnlyItem(EmsUtil.findProperties(project, key));
          if (emsProperty != null) {
            // Add a folding descriptor for the literal expression at this node.
            descriptors.add(new FoldingDescriptor(literalExpression.getNode(),
                new TextRange(literalExpression.getTextRange().getStartOffset() + 1,
                    literalExpression.getTextRange().getEndOffset() - 1),
                group, Collections.singleton(emsProperty)));
          }
        }
      }
    });
  ...
  /**
   * Gets the Ems Language 'value' string corresponding to the 'key'
   *
   * @param node Node corresponding to PsiLiteralExpression containing a string in the format
   *             EMS_PREFIX_STR + EMS_SEPARATOR_STR + Key, where Key is
   *             defined by the Ems language file.
   */
  @Nullable
  @Override
  public String getPlaceholderText(@NotNull ASTNode node) {
    if (node.getPsi() instanceof PsiLiteralExpression psiLiteralExpression) {
      ...
      String key = text.substring(EmsAnnotator.EMS_PREFIX_STR.length() +
          EmsAnnotator.EMS_SEPARATOR_STR.length());

      EmsProperty emsProperty = ContainerUtil.getOnlyItem(
          EmsUtil.findProperties(psiLiteralExpression.getProject(), key)
      );
      if (emsProperty == null) {
        return StringUtil.THREE_DOTS;
      }

      String propertyValue = emsProperty.getValue();
